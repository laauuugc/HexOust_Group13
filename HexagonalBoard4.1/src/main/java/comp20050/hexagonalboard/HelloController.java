...
public class HelloController {
    ...
    protected boolean playerTurn = false; // False means it's red player turn, true means it's blue player turn
    ...
    private void updateTurnLabel() {
        ...
        currentPlayerCircle.setFill(playerTurn ? BLUE : RED);
    ...
    @FXML // Label that appears when an invalid move is made
    ...
    void getHexID(MouseEvent event) {
        ...
        long playerCount = adjacent.stream().filter(h -> boardState.get(h)==playerColor).count();
        ...
        boardState.put(hexagon,playerColor); // Temporarily place the stone for group evaluation

        int newGroupSize = countGroupSize(hexagon,playerColor); // Count new group size

        // Find only the opponent groups that are adjacent to the new stone
        Set<Polygon> adjacentOpponentGroups = new HashSet<>();
        Set<Set<Polygon>> opponentGroupsToCheck = new HashSet<>();

        // Identify all opponent stones adjacent to the new stone
        for (Polygon neighbor : adjacent) {
            if (boardState.get(neighbor)==opponentColor) {
                // For each opponent stone,find its group
                Set<Polygon> group = new HashSet<>();
                collectGroup(neighbor,opponentColor,group);
                adjacentOpponentGroups.addAll(group);
                opponentGroupsToCheck.add(group);
            }
        }

        boolean playerCanCapture = true;
        for (Set<Polygon> group : opponentGroupsToCheck) {
            if (group.size() >= newGroupSize) {
                playerCanCapture = false;
                System.out.println("New group is NOT bigger than all opponent groups. No capture.");
                break;
            }
        }
        ...
        if (playerCount > 0 && opponentCount > 0) {
            ...
            placeStone(hexagon,playerColor);// Place valid capturing stone

            // Get the entire new group that the placed stone belongs to
            Set<Polygon> newGroup = new HashSet<>();
            collectGroup(hexagon,playerColor,newGroup);

            // Find and remove all opponent groups connected to ANY stone in the new group
            captureOpponentGroups(newGroup,playerColor);
    ...
    // Captures all opponent groups that are connected to any stone in the player's group
    private void captureOpponentGroups(Set<Polygon> playerGroup,Color playerColor) {
        Color opponentColor = (playerColor==RED) ? BLUE : RED;
        int newGroupSize = playerGroup.size();

        Set<Polygon> opponentStonesToCapture = new HashSet<>();
        Set<Set<Polygon>> processedGroups = new HashSet<>();

        // For each stone in the player's group
        for (Polygon stone : playerGroup) {
            // Get all neighboring opponent stones
            for (Polygon neighbor : getNeighbours(stone)) {
                if (boardState.get(neighbor)==opponentColor) {
                    // For each opponent stone,find its group and mark for capture
                    Set<Polygon> opponentGroup = new HashSet<>();
                    collectGroup(neighbor,opponentColor,opponentGroup);
                    // Add to processed groups to avoid checking the same group twice
                    boolean isNewGroup = true;
                    for (Set<Polygon> processedGroup : processedGroups) {
                        if (processedGroup.contains(neighbor)) {
                            isNewGroup = false;
                            break;
                        }
                    }
                    ...
                    if (isNewGroup) {
                        processedGroups.add(opponentGroup);

                        // Only capture if the opponent group is smaller than player's group
                        if (opponentGroup.size() < newGroupSize) {
                            opponentStonesToCapture.addAll(opponentGroup);
                        }
                    }
                }
            }
        ...
        // Remove all the opponent stones that were marked for capture
        List<Circle> stonesToRemove = new ArrayList<>();

        for (Polygon hex : opponentStonesToCapture) {
            boardState.remove(hex);
            hex.setFill(Color.rgb(216,176,238)); // Recolor the hexagon purple, so it looks empty
        ...
        System.out.println("Captured " + stonesToRemove.size() + " opponent stones");
    }

    private void showErrorPopup() {
        errorMessage.setText("Error: Invalid stone placement!");
        errorMessage.setVisible(true);

        PauseTransition pause = new PauseTransition(Duration.millis(3500));
        pause.setOnFinished(event -> {hideErrorMessage();});
        pause.play();
    ...
    private void placeStone(Polygon hexagon,Color color) {
        ...
        Circle stone = new Circle(centerX,centerY,20); // Radius 20 for a nice fit in the hexagon
        stone.setFill(color);
    ...
    private void checkForWinner() {
        ...
        // If one player has no stones left, the other player wins
        if (!hasRedStones) {
            winnerDeclared = true;
