...
class HelloControllerTest {
    ...
    public void setUp() throws Exception {
        ...
        injectField("currentPlayerCircle", new Circle());
    ...
    void testCapturingMoveRemovesOpponent() throws Exception {
        ...
        Polygon p3 = tester.getHexagons()[2];
        ...
        // Place RED on p3
        placeStone.invoke(tester,p3,HelloController.RED);

        // Get the methods needed for testing capture logic
        Method collectGroup = HelloController.class.getDeclaredMethod("collectGroup",Polygon.class,Color.class,Set.class);
        collectGroup.setAccessible(true);

        Method captureOpponentGroups = HelloController.class.getDeclaredMethod("captureOpponentGroups",Set.class,Color.class);
        captureOpponentGroups.setAccessible(true);
        // Create a set to hold the RED group
        Set<Polygon> redGroup = new HashSet<>();

        // Collect the RED group (should include p1 and p3)
        collectGroup.invoke(tester,p1,HelloController.RED,redGroup);

        // Invoke the capture logic
        captureOpponentGroups.invoke(tester,redGroup,HelloController.RED);
    }
}