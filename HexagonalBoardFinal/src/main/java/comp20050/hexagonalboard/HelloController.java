...
public class HelloController {
    private String namePlayer1 = "Player 1"; // Default name if not set
    private String namePlayer2 = "Player 2";
    protected boolean playerTurn = false; // false= Red's turn true= Blue's turn
    ...
    private void updateTurnLabel() {
        currentPlayer.setText(playerTurn ? namePlayer2 + "'s turn" : namePlayer1 + "'s turn");
    ...
    @FXML // Label that shows the current player's name and color
    ...
    void initialize() {
        ...
        currentPlayer.setText(namePlayer1 + "'s turn");
    ...
    void getHexID(MouseEvent event) {
        ...
        if (boardState.containsKey(hexagon)) // If a player already placed a stone
            showErrorMessage();
    ...
    private void removeStones(Polygon start,Color color) {
        Set<Polygon> toRemove = new HashSet<>(); //what is it to remove???
        ...
        for (Polygon hex : toRemove) {
            ...
            hex.setFill(Color.rgb(216,176,238)); // Recolor the hexagon purple, so it looks empty with no stone inside
    ...
    private void checkForWinner() {
        ...
        if (!hasRedStones) {
            winnerDeclared = true;
            displayWinnerInterface(namePlayer2,BLUE);
        } else if (!hasBlueStones) {
            winnerDeclared = true;
            displayWinnerInterface(namePlayer1,RED);
        }
    }
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            dialog.addWindowListener(new java.awt.event.WindowAdapter() {
                ...
                public void windowClosing(java.awt.event.WindowEvent e) {
                    System.exit(0); // If user clicks X button
                }
            });
